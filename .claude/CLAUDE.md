...
- 値をポインタ化するとき、簡潔に書けるなら `&` を優先する（例: `&corporationID`）。`&` で書けない／中間変数を導入しないと書けない場合は Go 1.26 の `new()` を使う（例: `new(f())`、`new("literal")`）。`v := getValue(); ptr := &v` のような中間変数 + `&` の 2 行パターンは避ける
- doc コメントでメソッド名・関数名を列挙しない（rename・追加・削除で腐るため）。代わりに不変条件や責務の所在を書く
...
- 更新系メソッドのテストでは、メソッド呼び出し前のエンティティを基準に `cmp.Diff` し、変更されるフィールドと変更されないフィールドの両方を一括で検証する。期待される変更だけでなく「他に副作用がない」ことも契約として保証する
