...
- コメントはまず「書かない」を既定にする。残すのは「この行が無いと有能な読み手が誤解する / 間違って直す」ときだけ。書く前にコード本体・呼び先の godoc・型・テスト・terraform 等で追えないか確認し、追えるなら消す。ライブラリ/基盤のデフォルト挙動を受け入れているだけの箇所や、誰も間違えない選択に理由を書かない（"Why not" を装った正当化に注意）
- 1つのコメント内で同じ語を別の対象に使わない。「通知」「キュー」「エラー」等の総称は、どれを指すか具体名で書く
- コメントの日本語部分に、馴染みのない専門用語や英単語（例: redrive）をそのまま書かない。何が起きるかを普通の日本語で説明し、どうしても用語を使うなら短い言い換えを添える（識別子・型名・SQL キーワード等はそのままでよい）。それっぽい機構名で精度を装わず、観測できる動作を書く
