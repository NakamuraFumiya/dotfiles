# User Preferences

- PRは常に `--draft` フラグ付きで作成する（`gh pr create --draft`）
- PR の body 先頭に関連チケットのリンクを見出し付きで記載する（例: `## Ticket\n[PROJ-123](https://example.com/PROJ-123)`）。チケット URL はコンテキストから判断する
- PR レビューコメントへの返信には対応コミットハッシュのリンクを含める（例: `[537c3c7e](https://github.com/{owner}/{repo}/commit/537c3c7e) で対応しました。`）。owner/repo は対象リポジトリから判断する

# Design

- 仕様に根拠のない制約を実装に入れない

# Code Style

- 1回しか使わない値を中間変数に入れない。関数の引数に直接渡す
- 値をポインタ化するとき、簡潔に書けるなら `&` を優先する（例: `&corporationID`）。`&` で書けない／中間変数を導入しないと書けない場合は Go 1.26 の `new()` を使う（例: `new(f())`、`new("literal")`）。`v := getValue(); ptr := &v` のような中間変数 + `&` の 2 行パターンは避ける
- doc コメントでメソッド名・関数名を列挙しない（rename・追加・削除で腐るため）。代わりに不変条件や責務の所在を書く
- 情報を書く場所を使い分ける（t_wada 原則）: コードには How / テストコードには What / コミットログには Why / コードコメントには Why not。コードコメントはコードから読み取れない判断（なぜ自明な代替案を採らなかったか）に絞り、How の言い換えは書かない
- コメントはまず「書かない」を既定にする。残すのは、その行が無いと有能な読み手が誤解する / 間違って直すときだけ。呼び先のドキュメント・型・テスト・設定で追える情報や、ライブラリ・基盤のデフォルトをそのまま受け入れているだけの箇所には書かない（"Why not" を装った正当化に注意）
- 1つのコメント内で同じ語を別の対象に使わない。総称（「通知」「キュー」「エラー」など）は、どれを指すか具体名で書く
- 馴染みのない専門用語・略語をコメントの説明文にそのまま置かず、何が起きるかを平易な言葉で書く。用語を使うなら短い説明を添える（識別子・型名・言語やツールのキーワードは除く）。それっぽい機構名で精度を装わず、観測できる動作を書く

# Go Project Rules

以下は Go プロジェクト固有のルール。プロジェクト側の CLAUDE.md に移行予定。

## Testing

- テストを書くとき、最短でパスさせることより「何を保証するか」を重視する
- テストケース名は caller perspective で「〜こと」で書く。複数の主要な保証を全て含め、観察可能なレイヤーを明示する
    - 正常系（複合効果）: `ユーザーを作成し、ウェルカムメールを送信すること`
    - 副作用失敗だが RPC 成功: `ログ送信が失敗しても、RPC としては成功すること`
    - 異常系: `データ取得に失敗した場合、RPC としては Internal エラーを返すこと`
- テスト手法（fixture/mock/stub）の選択理由を聞かれたら説明できるようにする
- モックで逃げずに、既存のテストパターンに倣う
- 更新系テストでは、リクエスト値を factory のデフォルト値と異なる値にして、「値が実際に更新された」ことを保証する（factory のデフォルトと同じ値だと、更新されたのか元の値が残っただけか区別できない）
- 更新系メソッドのテストでは、メソッド呼び出し前のエンティティを基準に `cmp.Diff` し、変更されるフィールドと変更されないフィールドの両方を一括で検証する。期待される変更だけでなく「他に副作用がない」ことも契約として保証する
- controller にエラーを返しうる外部呼び出し（repository, service 等）を追加したら、その呼び出しが失敗するケースのテストも必ず追加する
- mock の EXPECT 設定で `gomock.Any()` を使う場合、引数の検証が不要な明確な理由があること。具体的な値が分かっている場合はその値を指定する
- テストのアサーションでは個別フィールド比較（`if got.X != want.X`）より `cmp.Diff` を使う。
- テストの失敗メッセージ（`t.Errorf` / `t.Fatalf` 等）は英語で書く。テストケース名（`t.Run` の名前）は日本語「〜こと」のままでよい

## Architecture

- Repository は集約（Aggregate）を常に完全な状態で返すこと。パフォーマンスのために部分取得が必要な場合は QueryService パターンを使う
- エラーは発生場所でアプリケーションエラー（`errors.NewError`）にラップする。呼び出し元でのラップ忘れを防ぐため、ヘルパー関数・mapper 等でも標準エラーをそのまま返さない
- repository の書き込みメソッド（Update / BatchCreate 等）を呼ぶときは、その実装が複数 SQL 文（複数テーブル UPDATE / Delete-Insert / 子集約の同期など）を発行するか確認する。発行する場合は controller 層で `tx.DoInTransaction` で囲む。途中失敗時の整合性を担保するため
