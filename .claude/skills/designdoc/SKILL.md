...
### 前提・根拠の箇条書き（Context）の書き方

Context の各箇条は「読み手が実装判断を追うための前提」なので、次を守る：

- まず「何を参照している / 何が起きる」を平易な語で言い、識別子（列名・関数名）は文末の括弧に 1 つだけ添える。ファイルパスは文中に挟まず、落とすか文末に分ける
- 「キー」「エラー」「ID」などの汎用語は 1 つの対象にだけ使う。DB の一意制約・列の値・オブジェクトストレージのキーが同じ文にあるなら「制約」「値」「オブジェクトキー」と呼び分ける
- 因果は読み手に要る段だけ書く。「制約がある → 書くとエラー名 X になる → だからコピーする」の中間段階（エラー名・関数の内部挙動）は How なので Code and pseudo-code 側に置き、前提では「同じ値を 2 行に持てない」までにする
- 名詞止めにしない。各文に動詞を置く
- 前例を複数挙げるときは 1 行に詰めず子箇条に分け、それぞれ「何の前例か」を 1 文で言う
