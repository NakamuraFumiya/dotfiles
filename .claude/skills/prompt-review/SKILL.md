...
ユーザーの過去のAIエージェント対話履歴を分析し、技術理解度・プロンプティングパターン・AI依存度を推定してレポートを生成する。レポートは日本語で `~/prompt-reviews/prompt-review-YYYY-MM-DD.md` に書き出す。
...
1. `~/prompt-reviews/` ディレクトリが存在しない場合は Bash で `mkdir -p ~/prompt-reviews` を実行
2. Write ツールを使って `~/prompt-reviews/prompt-review-YYYY-MM-DD.md` に書き出す（YYYYMMDDは実行日）
