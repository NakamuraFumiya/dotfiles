...
- SSH リモートログインを有効化: **システム設定 → 一般 → 共有 → 「リモートログイン」をオン**
  - CLI の `sudo systemsetup -setremotelogin on` は、ターミナルアプリに
    フルディスクアクセスが無いと `requires Full Disk Access privileges` で失敗する。
    GUI トグルの方が確実
